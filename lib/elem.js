...
function elem(root, production, options) {
  return new Elem(root, production, options);
}
...
function Elem(root, production, options) {
  ...
  this.buildDirName = this.options.buildDirName;
  this.buildDir = path.join(this.root, this.buildDirName);
  ...
  this.lastModifyBuiltFilename = path.join(root, "/" + this.buildDirName + "_elem_last_build.json");
  try {
    this.lastModifyBuilt = JSON.parse( fs.readFileSync( this.lastModifyBuiltFilename ) );
  } catch (e) {
    if (e.code !== 'ENOENT') {
      throw e;
    }
...
Elem.prototype.loader = function(opts) {
  ...
  return function(req,res,next) {
    ...
    function handleFiles() {
      connect.static(self.buildDir)(req, res, next);
    }
...
Elem.prototype.build = function(basepath) {
  var root = this.root;
  var self = this;
  var elemLastBuildPath = this.lastModifyBuiltFilename;
